...
def PaveDroit(parametre):
    ...
    cor.append("$$ Volume = %s \\times %s \\times %s= %s $$" % (longueur,largeur,hauteur,longueur*largeur*hauteur))
    cor.append("$$ Volume = \\unit[%s]{%s} $$" % (longueur*largeur*hauteur,uniteVolume[unite]))
