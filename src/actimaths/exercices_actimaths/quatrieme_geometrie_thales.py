...
def tex_figure_thales(enonce,nom_ordre,nom_melange,coordonnee,coefficient):
    ...
    enonce.append("\\pstHomO[HomCoef=%s]{%s}{%s,%s}[%s,%s]" %(coefficient,nom_ordre[0],nom_ordre[1],nom_ordre[2],nom_ordre[3],nom_ordre[4]))
    # grand triangle
...
def ThalesQuotient(parametre):
    ...
    ## ---Calcul des paramètres---
    (nom_ordre,nom_melange,coordonnee) = caracteristique_sommet_thales()
    coefficient = random.uniform(0.25,0.75)
    ## ---Redaction---
    tex_figure_thales(exo,nom_ordre,nom_melange,coordonnee,coefficient)
    tex_figure_thales(cor,nom_ordre,nom_melange,coordonnee,coefficient)
